...
class AnalyticsViewModel @Inject constructor(
    ...
    private val _dateFilterState = MutableStateFlow(
        Pair(DateFilterType.THIS_MONTH, 0L to 0L)
    )
    ...
    val uiState: StateFlow<AnalyticsUiState> = combine(
        ...
        _uiState
    ) { transactions, tags, currentState ->
    ...
    fun onDateFilterChanged(filterType: DateFilterType) {
        if (filterType == DateFilterType.CUSTOM) {
            _uiState.update { it.copy(isFilterSheetVisible = false, isDateRangePickerVisible = true) }
        } else {
            _dateFilterState.value = Pair(filterType, 0L to 0L)
            _uiState.update { it.copy(dateFilter = filterType, isFilterSheetVisible = false) }
        }
    }

    fun onCustomDateRangeSelected(startDate: Long, endDate: Long) {
        _dateFilterState.value = Pair(DateFilterType.CUSTOM, startDate to endDate)
    ...
    private fun createPieData(transactions: List<TransactionWithCategoryAndTags>): PieData? {
        if (transactions.isEmpty()) return null
        val grouped = transactions.groupBy { it.category }
            .mapValues { entry -> entry.value.sumOf { it.transaction.amount } }
        val entries = grouped.map { (category, sum) -> PieEntry(sum.toFloat(), category?.name ?: "Uncategorized") }
        val colors = grouped.keys.map { category ->
    ...
    private fun createLineData(transactions: List<TransactionWithCategoryAndTags>): LineData? {
        if (transactions.isEmpty()) return null
        val sdf = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault())
        val groupedByDay = transactions.groupBy { sdf.format(Date(it.transaction.date)) }
        val expenseEntries = mutableListOf<Entry>()
        val incomeEntries = mutableListOf<Entry>()
        groupedByDay.keys.sorted().forEach { dateString ->
            val date = sdf.parse(dateString)?.time ?: 0L
            val dayTransactions = groupedByDay[dateString] ?: emptyList()
            val dailyExpense = dayTransactions.filter { it.transaction.type == "EXPENSE" }.sumOf { it.transaction.amount }.toFloat()
            val dailyIncome = dayTransactions.filter { it.transaction.type == "INCOME" }.sumOf { it.transaction.amount }.toFloat()
            if (dailyExpense > 0) expenseEntries.add(Entry(date.toFloat(), dailyExpense))
            if (dailyIncome > 0) incomeEntries.add(Entry(date.toFloat(), dailyIncome))
        }
        val expenseDataSet = LineDataSet(expenseEntries, "Expense").apply { color = Color.Red.toArgb(); valueTextColor = Color.Red.toArgb(); setCircleColor(Color.Red.toArgb()) }
        val incomeDataSet = LineDataSet(incomeEntries, "Income").apply { color = Color(0xFF3871C1).toArgb(); valueTextColor = Color(0xFF3871C1).toArgb(); setCircleColor(Color(0xFF3871C1).toArgb()) }
